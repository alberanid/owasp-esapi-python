...
class DefaultEncryptor(Encryptor):
    def __init__(self):
        self.hashAlgorithm = esapi.core.securityConfiguration().getHashAlgorithm();
        self.hashIterations = esapi.core.securityConfiguration().getHashIterations();

    def hash(self, plaintext, salt, iterations=self.hashIterations):
        try:
            digest = hashlib.new(self.hashAlgorithm)
            digest.update(esapi.core.securityConfiguration().getMasterSalt())
            digest.update(salt)
            ...
            #encoded = esapi.core.encoder().encodeForBase64(bytes, false)
            #return encoded
